...
function isGlobalInstall() {
  const flag = process.env.npm_config_global || process.env.BAICAI_VIBE_INSTALL_SCOPE;
...
function resolveBackupDir(targetDir) {
  return `${targetDir}.baicai-vibe-backup`;
}

function resolveLockPath(targetDir) {
  return `${targetDir}.baicai-vibe.lock`;
}

function isNonInteractive() {
  if (process.env.CI === 'true') return true;
  if (process.env.BAICAI_VIBE_FORCE === 'true') return true;
...
function hasBaicaiVibeContent(dir, paths = ownedPaths) {
