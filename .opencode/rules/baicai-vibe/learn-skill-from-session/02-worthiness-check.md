...
Use the extracted candidate set from the pattern-extraction step, source-session evidence, the explicit goal, current workspace artifacts, and existing stable artifacts under `.opencode/skills/baicai-vibe/`, `.opencode/workflows/baicai-vibe/`, and `.opencode/rules/baicai-vibe/`.

Apply the governing rules from `@.opencode/rules/baicai-vibe/learn-skill-from-session/candidate-rule.md` and `@.opencode/rules/baicai-vibe/learn-skill-from-session/promotion-rule.md`.
