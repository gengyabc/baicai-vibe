# Workflow: Excalidraw Draw

## Goal

Turn a user request into `.excalidraw` JSON files that argue visually, not just display information.

## Inputs

- The user request passed from `@.opencode/commands/baicai-vibe/excalidraw.md`
- Any target path, naming hint, or scope constraint in the request
- Reference files under `@.opencode/workflows/baicai-vibe/excalidraw/references/`

## Output

- Valid `.excalidraw` JSON file(s)
- PNG renders used to validate the result
- A short report of final output path(s)
...
```bash
cd .opencode/workflows/baicai-vibe/excalidraw/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
...
```bash
cd .opencode/workflows/baicai-vibe/excalidraw/references
uv sync
